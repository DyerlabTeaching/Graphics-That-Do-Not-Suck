...
```{r}
df |>
  ggplot( aes(Air_Temperature, Water_Temperature) ) + 
  geom_point() + 
  xlab("Air Temperture (°C)") +
  ylab("Water Temperture (°C)")
```
...
```{r}
df |>
  ggplot( aes(Air_Temperature, Water_Temperature, shape=Weekday) ) + 
  geom_point() + 
  xlab("Air Temperture (°C)") +
  ylab("Water Temperture (°C)")
```
...
```{r}
df |>
  ggplot( aes(Air_Temperature, Water_Temperature, shape=Weekday, color=PAR) ) + 
  geom_point() + 
  xlab("Air Temperture (°C)") +
  ylab("Water Temperture (°C)")
```
...
df |>
  group_by( Weekday ) |>
  summarize( Maximum = max(Air_Temperature ))
```
...
df |> 
  ggplot( ) + 
  geom_line( aes(Date, Air_Temperature), color="green" )  + 
